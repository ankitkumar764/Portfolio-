        ...
        <section id="about" style={{ padding: '120px 24px', background: 'transparent' }}>
            <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ marginBottom: '80px' }}>
                    <p style={{ fontFamily: "'Outfit', sans-serif", color: 'var(--gold)', fontSize: '13px', fontWeight: '600', letterSpacing: '4px', textTransform: 'uppercase', marginBottom: '16px' }}>About Me</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
                        <h2 style={{ fontSize: 'clamp(32px, 5vw, 48px)', fontWeight: '800', color: 'var(--text)', letterSpacing: '-1.5px' }}>My Approach.</h2>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '80px' }}>
                    ...
                    <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                        <p style={{ color: 'var(--text)', fontSize: '20px', lineHeight: '1.6', marginBottom: '24px', fontWeight: '500' }}>
                            Hi, I'm <span style={{ color: 'var(--gold)' }}>Ankit Singh</span>, pursuing my BE at Swaminarayan XCG.
                        </p>
                        ...
                        <p style={{ color: 'var(--text-mut)', fontSize: '15px', lineHeight: '1.8', marginBottom: '40px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                            {[
                                ...
                                <div key={item.label}>
                                    <p style={{ fontSize: '11px', color: 'var(--gold)', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '4px', fontWeight: '800' }}>{item.label}</p>
                                    <p style={{ color: 'var(--text)', fontWeight: '600', fontSize: '14px' }}>{item.value}</p>
                                </div>
                            ))}
                        </div>
                    </motion.div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                        {stats.map((s, i) => (
                            <motion.div key={i} whileHover={{ y: -8, borderColor: 'rgba(212, 175, 55, 0.5)', boxShadow: '0 10px 30px rgba(212, 175, 55, 0.15)' }}
                                ...
                                <div style={{ fontSize: '40px', fontWeight: '800', color: 'var(--text)', marginBottom: '8px', fontFamily: "'Outfit', sans-serif" }}>{s.value}</div>
                                <div style={{ fontSize: '11px', color: 'var(--text-mut)', textTransform: 'uppercase', letterSpacing: '2px', fontWeight: '700' }}>{s.label}</div>
                            </motion.div>
                        ))}
                    </div>
                </div>
